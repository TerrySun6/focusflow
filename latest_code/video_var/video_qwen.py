...
# ==================== 全局配置 ====================
YOUTUBE_URL = "https://www.youtube.com/watch?v=lZ3bPUKo5zc&t=1524s"
WHISPER_MODEL = "mlx-community/whisper-small-mlx-4bit"
SBERT_MODEL_NAME = "mlx-community/Qwen3-Embedding-0.6B-8bit"
...
def download_audio(url, output_path):
    ...
    ydl_opts = {
        ...
        "quiet": False,
        "cookiesfrombrowser": ("safari",),  # 从 Safari 提取已登录 YouTube 的 cookies
        "remote_components": REMOTE_COMPONENTS,
        "js_runtimes": JS_RUNTIMES,
    }
    ...
    def run_pelt_segmentation(self):
        ...
        if n < 5:
            self.results["segments"] = {
                "boundaries": [0, n], "topics": [0], 
                "segment_ids": np.zeros(n, dtype=int), "method": "too_short"
            }
            return
...
def load_or_embed(chunks, output_dir):
    """
    针对 8GB 内存 M3 Mac 优化的 Qwen3 编码函数
    支持自动显存清理与维度截断
    """
    import gc
    import os
    ...
    try:
        ...
        print("❌ 错误: 未安装 mlx-lm。请运行: pip install mlx-lm")
        raise
    ...
    if os.path.exists(emb_cache):
        ...
        print(f"✅ 已读取 embeddings 缓存: {emb_cache} | 形状: {embeddings.shape}")
        return torch.tensor(embeddings, dtype=torch.float32)

    print(f"🚀 正在加载模型: {SBERT_MODEL_NAME}")
    
    # 2. 加载模型
    model, tokenizer = load(SBERT_MODEL_NAME)
    
    all_embeddings = []
    total_chunks = len(chunks)
    
    # 内存管理配置
    CLEAR_EVERY = 10 # 每处理 20 个 chunk 强制清理一次内存
    
    print(f"🧪 开始编码 {total_chunks} 个语义块...")
    
    try:
        for i, chunk in enumerate(chunks):
            # 编码文本（加上指令引导以优化逻辑表征）
            full_prompt = f"Represent this passage for logical structure analysis: {chunk}"
            tokens = mx.array(tokenizer.encode(full_prompt))
            
            # 前向传播：获取隐藏层状态 [1, seq_len, 1024]
            output = model.model(tokens[None])
            
            # 平均池化：将序列压缩为向量 [1024]
            emb = mx.mean(output, axis=1)
            
            # 【关键】强制执行计算，防止内存堆积
            mx.eval(emb)
            
            # 转换为 NumPy 并存入 CPU 内存，同时利用套娃特性截断到 256 维
            # 256 维在保持 Qwen3 约 98% 精度的同时，能让 PHATE 运行速度提升 4 倍
            all_embeddings.append(np.array(emb).flatten()[:256])
            
            # 定期清理显存
            if (i + 1) % CLEAR_EVERY == 0:
                mx.metal.clear_cache()
                gc.collect()
                print(f"  进度: {i + 1}/{total_chunks} | 内存已回收")
        ...
        print(f"✅ 编码完成，缓存已保存: {emb_cache}")
        
        # 4. 彻底释放模型显存，为后续 PHATE 算法腾空间
        del model
        gc.collect()
        mx.metal.clear_cache()
