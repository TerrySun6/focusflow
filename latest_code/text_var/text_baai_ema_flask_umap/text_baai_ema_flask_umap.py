"""
Flask UI for text_baai_ema.py — UMAP 替代版
================================================================
在原有 EMA + UMAP 降维管线基础上，引入策略 B（零均值中心化 Centering）。
在 EMA 平滑之后、进入下游任务（UMAP 降维、Leiden/HDBSCAN 聚类、PELT 分段）
...
class EMAnalyzer:
    ...
    def run_umap(self) -> Optional[np.ndarray]:
        ...
        coords = reducer.fit_transform(self.embeddings_cpu)
        self.results["umap"] = coords
        return coords
...
def _make_cache_key(path: str, title: str, cfg: Config) -> str:
    """生成分析结果的缓存键。"""
    raw = f"{path}|{title}|{cfg.ema_alpha}|{cfg.ema_bidirectional}|{cfg.ema_window_size}|{cfg.umap_n_neighbors}|{cfg.umap_metric}|{cfg.umap_min_dist}|{cfg.pelt_penalty_multiplier}|{cfg.leiden_resolution}|{cfg.hdbscan_min_cluster_size}"
...
def _export_umap_html(umap_fig: go.Figure, out_dir: str, title: str) -> str:
    """将已构建好的 UMAP 3D Plotly 图形导出为自包含交互式 HTML 文件。

    用浏览器打开后可以旋转、缩放、悬停查看每点标注，保持完整的交互性。
    """
    html_path = os.path.join(out_dir, "umap_3d.html")
    ...
    logger.info("📦 UMAP 3D 交互图已导出 -> %s", html_path)
    return html_path
...
def _run_analysis_pipeline(path: str, title: str, cfg: Config) -> Dict[str, Any]:
    ...
    try:
        ...
        # UMAP 和聚类互相独立，并行执行以减少端到端延迟
        with ThreadPoolExecutor(max_workers=2) as pool:
            fut_umap = pool.submit(analyzer.run_umap)
            fut_cluster = pool.submit(analyzer.run_clustering)
            umap_coords = fut_umap.result()
        ...
        if umap_coords is not None:
            ...
            for s_i in range(max(0, len(bs) - 1)):
                ...
                umap_fig.add_trace(go.Scatter3d(
                    x=umap_coords[idx, 0], y=umap_coords[idx, 1], z=umap_coords[idx, 2],
            ...
            b_idx = [max(0, min(int(b), len(umap_coords)-1)) for b in bs]
            umap_fig.add_trace(go.Scatter3d(
                x=umap_coords[b_idx, 0], y=umap_coords[b_idx, 1], z=umap_coords[b_idx, 2],
            ...
            umap_fig.add_trace(go.Scatter3d(
                ...
                text=["START"], name="起点"))
            umap_fig.add_trace(go.Scatter3d(
                x=[umap_coords[-1, 0]], y=[umap_coords[-1, 1]], z=[umap_coords[-1, 2]],
                mode="markers+text", marker=dict(size=8, color="green", symbol="circle"),
                text=["END"], name="终点"))
            umap_fig.update_layout(
                title=f"UMAP 3D — α={cfg.ema_alpha} {'双向' if cfg.ema_bidirectional else '单向'}",
            ...
            result["umap_chart_json"] = plotly.io.to_json(umap_fig)

            # ── 导出自包含交互式 HTML（后台线程，不阻塞 HTTP 响应）──
            threading.Thread(target=_export_umap_html, args=(umap_fig, doc_dir, title), daemon=True).start()

            # 轨迹统计
            total_dist = float(np.sum(np.linalg.norm(umap_coords[1:] - umap_coords[:-1], axis=1)))
            end_dist = float(np.linalg.norm(umap_coords[0] - umap_coords[-1]))
            result["trajectory"] = {
                "total_dist": total_dist,
                "end_dist": end_dist,
                "ratio": total_dist / max(end_dist, 1e-9),
            }
            result["umap_html_file"] = os.path.join(doc_dir, "umap_3d.html")
...
def analyze():
    ...
    cfg.umap_n_neighbors = int(data.get("umap_n_neighbors", 5))
    cfg.umap_metric = str(data.get("umap_metric", "cosine"))
    cfg.umap_min_dist = float(data.get("umap_min_dist", 0.1))
...
if __name__ == "__main__":
    ...
    print(f"✨ UMAP 替代 PHATE；EMA + Centering 保留")
